 ...
 */
async function simulateKeyPress(element, { key, code, keyCode, ctrlKey = false, metaKey = false, shiftKey = false }) {
...
async function getPageContext() {
    ...
    if (activeElement) {
        ...
        if (activeElement.isContentEditable) {
            console.log("[getContext] Getting content via Select All + Copy simulation...");
            ...
            try {
                ...
                if (!copyCommandSuccess) {
                    console.warn("[getContext] execCommand('copy') returned false. The browser might have blocked it, or the page intercepted it and failed.");
                }

                // 4. Read from Clipboard
                console.log('[getContext] Attempting to read from clipboard (requires clipboardRead permission)...');
                try {
                    contentFromClipboard = await navigator.clipboard.readText();
                    console.log(`[getContext] Content read from clipboard (${contentFromClipboard.length} chars).`);
                    ...
                    if (clipboardError.name === 'NotAllowedError') {
                        console.error(">>>>> Ensure 'clipboardRead' permission is in manifest.json and reload extension! User may need to grant permission on first use.");
                ...
                if(originalRanges.length > 0) {
                    originalRanges.forEach(range => {
                      try { selection.addRange(range); } catch (e) { console.warn("Couldn't restore a range", e)}
                    });
                } else {
                ...
                if (hadFocus && document.activeElement !== activeElement) {
                  ...
                  if(originalRanges.length > 0) {
                      originalRanges.forEach(range => {
                        try { selection.addRange(range); } catch (e) { console.warn("Couldn't restore a range post-focus", e)}
                      });
                  }
                }
...
async function insertText(text, replaceAll = true) {
    console.log('insertText function called with text:', text, 'and replaceAll:', replaceAll);
    const activeElement = document.activeElement;
    if (!activeElement) {
    ...
    try {
        ...
        // --- Handle ContentEditable Elements ---
            ...
            if (replaceAll) {
                console.log(`replaceAll is true. Simulating ${isMac ? 'Cmd+A' : 'Ctrl+A'} + Delete...`);
                ...
                if (currentContent.trim() === '') {
                    console.log(' Verification: Element seems empty after Delete simulation.');
                } else {
                    console.warn(` Verification: Element still contains text (${currentContent.length} chars) after Delete simulation.`);
                }
            ...
            try {
                ...
                setTimeout(() => { activeElement.dispatchEvent(new Event('input', { bubbles: true, cancelable: true })); }, 0);
            ...
            if (!insertionSuccess && !replaceAll) {
                console.log('Paste sim failed. Trying document.execCommand("insertText")...');
            ...
            if (!insertionSuccess) {
                 ...
                 console.log('Paste/insertText failed. Trying document.execCommand("insertHTML")...');
            ...
            if (!insertionSuccess && replaceAll) {
                console.warn('All insertion methods failed after key simulation. Falling back to direct innerHTML replacement.');
                const htmlText = text.replace(/\n/g, '<br>');
                activeElement.innerHTML = htmlText;
                // Attempt to move cursor to end
                 try {
                    const range = document.createRange(); const sel = window.getSelection();
                    range.selectNodeContents(activeElement); range.collapse(false);
                    sel.removeAllRanges(); sel.addRange(range);
                } catch(e) { console.error("Error setting cursor after innerHTML", e); }
                insertionSuccess = true;
            }

            if (!insertionSuccess) {
                 console.error('All methods failed to clear/insert text in contentEditable.');
            }
