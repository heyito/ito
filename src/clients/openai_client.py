...
class AudioInputTrack(AIORTCAudioStreamTrack): # Renamed to avoid conflict with aiortc's AudioStreamTrack
    """
    ...
    def __init__(self, audio_queue: asyncio.Queue, sample_rate: int, channels: int):
        ...
        logger.info(f"AudioInputTrack initialized: Rate={sample_rate}, Channels={channels}, Samples/Frame={self._samples_per_frame}, Bytes/Frame={self._bytes_per_frame}")
    ...
    async def recv(self) -> AudioFrame:
        ...
        while len(self._input_buffer) < self._bytes_per_frame:
             # logger.debug(f"Buffer has {len(self._input_buffer)} bytes, need {self._bytes_per_frame}. Waiting for queue...")
             get_start_time = time.monotonic()
             chunk = await self._queue.get()
             get_end_time = time.monotonic()
             # logger.debug(f"[{get_end_time:.3f}] Got chunk from queue in {(get_end_time - get_start_time)*1000:.2f} ms. Chunk type: {type(chunk)}")
             if chunk is None:
                 logger.info("Received None (EOS signal) from audio queue. Stopping track.")
             ...
             if isinstance(chunk, np.ndarray):
                  if chunk.dtype == np.int16:
                       ...
                       if np.issubdtype(chunk.dtype, np.floating):
                            ...
                            logger.warning(f"Received unexpected numpy array dtype: {chunk.dtype}. Attempting astype(np.int16).")
                            chunk_int16 = chunk.astype(np.int16)
                            self._input_buffer.extend(chunk_int16.tobytes())
             elif isinstance(chunk, bytes):
                 self._input_buffer.extend(chunk)
             else:
                 logger.warning(f"Received unexpected data type from queue: {type(chunk)}")

             # logger.debug(f"Added {len(chunk.tobytes() if isinstance(chunk, np.ndarray) else chunk)} bytes to buffer. Total: {len(self._input_buffer)}")
        ...
        # logger.debug(f"Extracted frame ({len(frame_data)} bytes). Buffer remaining: {len(self._input_buffer)} bytes.")
        ...
        new_frame = AudioFrame(
            ...
            layout='mono' if self._channels == 1 else 'stereo', # Set layout based on channels
        ...
        new_frame.time_base=Fraction(1, self._sample_rate) # Use sample rate for time base
        # Manually set pts based on monotonic time (more robust)
        new_frame.pts = int((now - self._start_time) / new_frame.time_base) # Calculate PTS based on elapsed monotonic time

        recv_end_time = time.monotonic()
        logger.debug(f"[{(recv_end_time):.3f}] recv finished in {(recv_end_time - recv_start_time)*1000:.2f} ms. Frame PTS: {new_frame.pts}")
...
class OpenAIWebRTCClient:
    ...
    async def connect(self, audio_queue: asyncio.Queue, sample_rate: int, channels: int):
        """
        ...
        if not self._audio_queue or not self._sample_rate or not self._channels:
             logger.error("Audio queue, sample rate, and channels must be provided for connect.")
             raise ValueError("Audio queue, sample rate, and channels must be provided for connect.")
        ...
        try:
            ...
            logger.info(f"Connection attempt finished. State: {self.pc.connectionState}")


    async def _establish_connection(self):
        """Internal method to handle WebRTC negotiation."""
        if not self._audio_queue or not self._sample_rate or not self._channels:
             logger.error("Audio source details missing during connection establishment.")
             raise ConnectionError("Audio source not configured before connection.")

        # 1. Create and add the custom audio track
        self.audio_track = AudioInputTrack(self._audio_queue, self._sample_rate, self._channels)
        self.pc.addTrack(self.audio_track)
        logger.info(f"Added AudioInputTrack (Rate: {self._sample_rate}, Channels: {self._channels}).")
        ...
        async def on_message(message: str):
            # print(f"RAW DataChannel Message Received: {message[:200]}...") 
            try:
                ...
                if event_type == "conversation.item.input_audio_transcription.delta":
                    print(f"delta received: {data.get("delta")}")
                ...
                if transcript_text and self._on_transcription_received:
                    ...
                    if asyncio.iscoroutinefunction(self._on_transcription_received):
                        asyncio.create_task(self._on_transcription_received(callback_data))
                    else:
        ...
        logger.info("Created and set local SDP offer (including data channel 'oai-events').") 
        ...
        try:
            # Send SDP text
            async with session.post(target_url, data=request_body, headers=headers, ssl=ssl.SSLContext()) as response:
                logger.info(f"Received response status: {response.status}")
                response_text = await response.text() # Read text for debugging/error cases
                if response.status >= 400:
                    logger.error(f"Error Response Text: {response_text}")
                response.raise_for_status() # Raise HTTPError for bad responses (4xx or 5xx)
                
                # --- Expect SDP Answer Text, NOT JSON --- 
                logger.info("Received successful SDP answer response from session endpoint.")
                ...
                logger.info("ICE candidates should be handled automatically by aiortc from SDP answer.")
            ...
            logger.error(f"HTTP Error during session negotiation: {e.status} {e.message}")
            logger.error(f"Response text was: {await response.text()}") # Log response text on error
            ...
            logger.error(f"Unexpected error during WebRTC connection: {e}", exc_info=True) # Log traceback
    ...
    async def _wait_for_connection(self):
        ...
        while self.pc.connectionState != "connected":
            if self.pc.connectionState in ["failed", "closed", "disconnected"]:
                logger.error(f"Connection entered failed/closed state while waiting: {self.pc.connectionState}")
                raise ConnectionError(f"WebRTC connection failed. State: {self.pc.connectionState}")
    ...
    async def close(self):
        ...
        if self.audio_track and hasattr(self.audio_track, 'stop'):
             ...
             logger.debug("Audio track is None or has no stop method, skipping stop call.")
