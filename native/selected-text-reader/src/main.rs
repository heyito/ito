...
mod tests {
    ...
    #[test]
    fn test_command_deserialization_get_text() {
        let json = r#"{"command":"get-text","format":"plain","maxLength":5000,"requestId":"req123"}"#;
        ...
        match cmd {
            Command::GetText {
                ...
                assert_eq!(format, Some("plain".to_string()));
                assert_eq!(max_length, Some(5000));
    ...
    #[test]
    fn test_command_deserialization_get_text_minimal() {
        let json = r#"{"command":"get-text","requestId":"req456"}"#;
        let cmd: Command = serde_json::from_str(json).unwrap();
    ...
    #[test]
    fn test_command_deserialization_get_cursor_context() {
        let json = r#"{"command":"get-cursor-context","contextLength":20,"cutCurrentSelection":true,"requestId":"ctx789"}"#;
        let cmd: Command = serde_json::from_str(json).unwrap();
        ...
        match cmd {
            Command::GetCursorContext {
                ...
                assert_eq!(context_length, Some(20));
                assert_eq!(cut_current_selection, Some(true));
                assert_eq!(request_id, "ctx789");
            }
            _ => panic!("Expected GetCursorContext command"),
        }
    }

    #[test]
    fn test_selected_text_response_success_serialization() {
        let response = SelectedTextResponse {
        ...
        assert_eq!(parsed["requestId"], "req123");
    ...
    #[test]
    fn test_selected_text_response_error_serialization() {
        let response = SelectedTextResponse {
            request_id: "req456".to_string(),
            success: false,
            text: None,
            error: Some("Failed to access clipboard".to_string()),
            length: 0,
        };
        ...
        assert_eq!(parsed["requestId"], "req456");
        assert_eq!(parsed["success"], false);
        assert_eq!(parsed["text"], serde_json::Value::Null);
        assert_eq!(parsed["error"], "Failed to access clipboard");
        assert_eq!(parsed["length"], 0);
    }

    #[test]
    fn test_cursor_context_response_serialization() {
        let response = CursorContextResponse {
            request_id: "ctx789".to_string(),
            success: true,
        ...
        assert_eq!(parsed["requestId"], "ctx789");
        assert_eq!(parsed["success"], true);
        assert_eq!(parsed["contextText"], "context");
        assert_eq!(parsed["error"], serde_json::Value::Null);
    ...
    fn test_response_with_special_characters() {
        let response = SelectedTextResponse {
            ...
            success: true,
            text: Some("Text with \"quotes\" and\nnewlines\ttabs".to_string()),
            error: None,
            length: 37,
        };
        ...
        assert_eq!(parsed["text"], "Text with \"quotes\" and\nnewlines\ttabs");
    }
}
