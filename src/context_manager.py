...
class ContextManager:
    ...
    def __init__(self, context_engine: ContextEngine):
        ...
        self._current_context: Dict[str, Optional[str]] = {"app_name": None, "doc_text": None}
        self._fetch_thread: Optional[threading.Thread] = None
        self._lock = threading.Lock() # Protects access to _current_context and _fetch_thread
    ...
    def fetch_context_async(self) -> None:
        ...
        with self._lock:
            ...
            if not active_window_info or not active_window_info.get("app_name"):
                print(f"[{timestamp}] ContextManager: Could not determine active window.")
            ...
            print(f"[{timestamp}] ContextManager: Active app '{app_name}'. Starting fetch thread...")
    ...
    def _fetch_target(self, app_name: str) -> None:
        ...
        try:
            ...
            if fetched_context is None:
                print(f"[{time.strftime('%H:%M:%S')}] ContextManager: Engine returned None for '{app_name}'.")
        except Exception as e:
            error_occurred = True
            print(f"[{time.strftime('%H:%M:%S')}] ContextManager: Error fetching context for '{app_name}': {e}")
            traceback.print_exc()
        finally:
            with self._lock:
                ...
                if threading.current_thread() == self._fetch_thread:
                    ...
                    self._current_context['doc_text'] = fetched_context
                     ...
                     print(f"[{time.strftime('%H:%M:%S')}] ContextManager: Stale fetch result ignored.")

            fetch_duration = time.time() - fetch_start_time
            status = "failed" if error_occurred or fetched_context is None else "succeeded"
            print(f"[{time.strftime('%H:%M:%S')}] ContextManager: Fetch thread finished ({status}, {fetch_duration:.3f}s).")
    ...
    def wait_for_context(self, timeout: float = 5.0) -> Optional[str]: 
        ...
        if thread_to_wait and thread_to_wait.is_alive():
            timestamp = time.strftime('%H:%M:%S')
            print(f"[{timestamp}] ContextManager: Waiting for context fetch (timeout: {timeout}s)...")
            thread_to_wait.join(timeout=timeout)
            if thread_to_wait.is_alive():
                print(f"[{timestamp}] ContextManager: Warning: Context fetch timed out.")
