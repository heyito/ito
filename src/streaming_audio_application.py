...
class StreamingAudioApplication(ApplicationInterface):
    ...
    def __init__(
        ...
        self.stop_audio_capture_event = (
            threading.Event()
        )  # Event to signal audio capture thread
        ...
        self.context_fetch_thread_handle: threading.Thread | None = (
            None  # Added for background context fetching
        )
        ...
        self.audio_stream_queue: asyncio.Queue | None = (
            None  # Audio: AudioHandler -> VoskProcessor
        )
        self.transcript_queue: asyncio.Queue | None = (
            None  # Transcript: VoskProcessor -> App
        )
        ...
        if not vosk_config or "model_path" not in vosk_config:
            raise ValueError(
                "Configuration Error: Missing 'model_path' under [Vosk] section in the provided configuration."
            )
    ...
    async def _consume_transcripts(self):
        ...
        while True:
            try:
                ...
                if not current_q:
                    logger.warning(
                        "Transcript queue became None during consumption loop iteration."
                    )
                ...
                if result is None:  # End signal
                    logger.info(
                        "Received None from transcript queue. Exiting consumer task."
                    )
                    break
                ...
                if is_final:
                    ...
                    print(
                        "\r" + text + " " * 20, end="", flush=True
                    )  # Use end and flush for better live update
    ...
    def _start_streaming_process(self) -> None:
        ...
        self.audio_stream_queue = asyncio.Queue(
            maxsize=200
        )  # Audio Capture -> Vosk Processor
        self.transcript_queue = asyncio.Queue(
            maxsize=100
        )  # Vosk Processor -> Transcript Consumer
        ...
        try:
            ...
            if self.audio_capture_thread_handle:
                self.audio_capture_thread_handle.join(timeout=1.0)
        ...
        if self.vosk_processor:
            ...
            if (
                ...
                asyncio.run_coroutine_threadsafe(
                    self.audio_stream_queue.put(None), self.asyncio_loop
                )
    ...
    def _llm_processing_thread_target(
        self, final_transcript: str, original_doc_context: str | None
    ) -> None:
