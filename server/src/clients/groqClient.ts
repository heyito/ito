...
class GroqClient {
   ...
   */
  public async transcribeAudio(audioBuffer: Buffer, fileType: string = 'webm'): Promise<string> {
    if (!this.isAvailable) {
    ...
    try {
      console.log(`Transcribing ${audioBuffer.length} bytes of audio using model ${this._asrModel}...`);
      ...
      throw new Error(`Groq API Error: ${error.message || 'An unknown error occurred'}`);
    }
  }
...
if (!process.env.GROQ_API_KEY || !process.env.GROQ_TRANSCRIPTION_MODEL) {
  console.error("FATAL: GROQ_API_KEY or GROQ_TRANSCRIPTION_MODEL is not set in the .env file. The application cannot start.");
  process.exit(1);
}
...
if (!apiKey) {
  console.error("FATAL: GROQ_API_KEY is not set in the .env file. The application cannot start.");
  process.exit(1);
}
