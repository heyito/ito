...
function getBinaryPath(): string | null {
    ...
    const binaryName = platform === 'win32' ? 'audio-recorder.exe' : 'audio-recorder'
    const baseDir = isDev 
    ...
    if (isDev) {
        if (platform === 'darwin') {
            ...
            log.error(`Unsupported development platform for audio-recorder: ${platform}`);
            return null;
        }
    } else {
...
export function startAudioRecorder() {
    ...
    try {
        audioRecorderProcess = spawn(binaryPath, [], { stdio: ['pipe', 'pipe', 'pipe'] })
...
function sendCommand(command: object) {
    if (audioRecorderProcess && audioRecorderProcess.stdin) {
        ...
        log.warn('Attempted to send command, but audio recorder is not running or stdin is not available.')
    }
}
