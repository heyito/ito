    ...
    def _handle_toggle(self):
        ...
        if self.audio_streamer.is_streaming:
            ...
            if (
                ...
                and not self._transcript_consumer_task.done()
            ):
            ...
            if self.audio_streamer.start_streaming():
                ...
                if self.audio_streamer.transcript_queue:
                    ...
                    logger.error(
                        "AudioStreamer started but transcript_queue is missing!"
                    )
            ...
            try:
                ...
                if is_final:
                    ...
                    if final_transcript:
                        ...
                        logger.info(
                            "Stopping stream components after final transcript..."
                        )
                        ...
                        logger.info(
                            f"Context ready (App: {current_context_data.get('app_name')})."
                        )

                        # 3. Process Command
                        logger.info("Initiating command processing...")
                        self.command_processor.process_command(
                            current_context_data, final_transcript
                        )
